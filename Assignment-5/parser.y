...
%token<symbol> IDENTIFIER               /* identifier->points to symbol table entry */
...
primary_expression: 
                    IDENTIFIER 
                        { 
                            ...
                            $$ = new Expression();                      // creates new non boolean expression
                            $$->symbol = $1;
...
postfix_expression:
                    primary_expression
                        { 
                            ...
                            $$ = new Array();   // creates new array with the same symbol as the primary expression
                            $$->symbol = $1->symbol;
                            $$->temp = $$->symbol;
                    ...
                    | postfix_expression LEFT_SQUARE_BRACKET expression RIGHT_SQUARE_BRACKET
                        { 
                            ...
                            $$->type = Array::ARRAY;    // type is set to array
                    ...
                    | unary_operator cast_expression
                        { 
                            ...
                            if(strcmp($1, "&") == 0) {
                                ...
                                $$->symbol->type->arrayType = $2->symbol->type;     // set the subArray type to the symbol type of the cast_expression
...
/*
1. Extract base type of the array and obtain value by indexing 
2. Apply operation
*/
multiplicative_expression:
                    cast_expression
...
/*
-> Boolean expression is made
-> Truelist and Falselist are made for later use and backpatched with correct destinations
*/
relational_expression:
                    shift_expression
                        { 
...
/*
-> Non boolean expression is made and type conversion is done
-> required operations are applied and result is stored in newly generated temporary
*/
...
/*
Markers M, N
M->stores location of quad generated at M
N->nextlist
*/
M:  
        {
...
assignment_expression:
                ...
                | unary_expression assignment_operator assignment_expression
                    { 
                        ...
                        if($1->type == Array::ARRAY) {
                            $3->symbol = $3->symbol->convert($1->subArrayType->type);// assignment to array
...
pointer:
        MULTIPLY type_qualifier_list_opt
            { 
                ...
                $$ = new SymbolType(SymbolType::POINTER); // new pointer type
            }
        | MULTIPLY type_qualifier_list_opt pointer
            { 
                yyinfo("pointer => * type_qualifier_list_opt pointer"); 
                $$ = new SymbolType(SymbolType::POINTER, $3); // nested pointer type
            }
        ;
...
// change symbol table on entering a new block
change_block: 
            {
                string name = currentTable->name + "_" + toString(tblCnt);
                tblCnt++;
...
function_definition: 
                    // to prevent block change here which is there in the compound statement grammar rule
